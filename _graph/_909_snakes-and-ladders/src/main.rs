...
impl Solution {
    pub fn snakes_and_ladders(board: Vec<Vec<i32>>) -> i32 {
        let mut vist = vec![0; board.len() * board.len() + 1];
        let mut queue: VecDeque<(usize, usize)> = VecDeque::new();
        queue.push_back((1, 0));
        while !queue.is_empty() {
            let point = queue.pop_front().unwrap();
            for i in 1..=6 {
                let mut next = point.0 + i;
                if next > board.len() * board.len() {
                    break;
                }
                let rc = Self::id2rc(next, board.len());

                if board[rc.0][rc.1] > 0 {
                    next = board[rc.0][rc.1] as usize;
                }

                if next == board.len() * board.len() {
                    return (point.1 + 1) as i32;
                }

                if vist[next] == 0 {
                    vist[next] = 1;
                    queue.push_back((next, point.1 + 1));
                }
            }
        }
        -1
    ...
    fn id2rc(id: usize, n: usize) -> (usize, usize) {
        let (r, mut c) = ((id - 1) / n, (id - 1) % n);
