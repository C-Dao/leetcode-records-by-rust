...
impl Solution {
    pub fn count_sub_trees(n: i32, edges: Vec<Vec<i32>>, labels: String) -> Vec<i32> {
        ...
        Self::dfs(0, -1, &adj, &labels.as_bytes(), &mut ans);
    ...
    fn dfs(
        ...
        label_count[(labels[node as usize] - b'a') as usize] = 1;
        ...
        for child in adj[node as usize].iter() {
            ...
            let child_label_counts = Self::dfs(*child, node, adj, labels, ans);

            for i in 0..26 {
                label_count[i] += child_label_counts[i];
            }
        }

        ans[node as usize] = label_count[(labels[node as usize] - b'a') as usize];
