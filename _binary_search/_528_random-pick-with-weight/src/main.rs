...
impl Solution {
    ...
    fn pick_index(&self) -> i32 {
        ...
        let index = rng.gen_range(0, self.total);

        let (mut left, mut right) = (0, self.sums.len() - 1);

        while left <= right {
            let mid = (left + right) / 2;

            if self.sums[mid] > index && (mid == 0 || self.sums[mid - 1] <= index) {
                return mid as i32;
            };

            if self.sums[mid] > index {
                right = mid - 1;
            } else {
                left = mid + 1;
            }
...
fn main() {
    let solution = Solution::new(vec![4, 2]);
